...
define([
...
], function (_, __, Backbone, groupTemplate, BaseForm, mediator, {QuickExportConfigurator}, UserContext, FetcherRegistry) {
  const ActionsPanel = BaseForm.extend({
    ...
    renderActions: function () {
      var wkModules = FetcherRegistry.getFetcher('wk_modules_finder').fetchAll();
      var self = this; 
      Promise.all([wkModules]).then(function(values) {
          self.$el.empty();

          var simpleLaunchers = _.filter(self.launchers, function (launcher) {
            return undefined === launcher.getGroup();
          });
          var groupedLaunchers = _.filter(self.launchers, function (launcher) {
            return undefined !== launcher.getGroup();
          });

          if (simpleLaunchers.length) {
            _.each(simpleLaunchers, launcher => {
              self.$el.append(launcher.render().$el);
            });
          }

          if (groupedLaunchers.length) {
            self.renderGroupedLaunchers(groupedLaunchers, values);
          }

          if (self.appendToGrid) {
            self.gridElement.prepend(self.$el);
          }
        });   
     ...
     * @return {*}
     */
    renderGroupedLaunchers: function (launchers, values) {
